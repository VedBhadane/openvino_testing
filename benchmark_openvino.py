...
The table has one row per "thing to report" and columns: Baseline system | NEW system | Delta %.
    ...
    Inference engine & version  The runtime doing the math = PyTorch (CPU / oneDNN). Version
                              is auto-printed.
    KPI 1  Throughput         How MUCH work per second. Here: inference samples processed
                              per second under sustained load. Higher = better.
    KPI 2  Latency            How LONG a single request takes. Here: time for one
                              single-sample inference, in milliseconds. Lower = better.
...
- Libraries assumed present in the target environment: torch, numpy. psutil is optional
  (only used to enrich the system-info banner).
"""
...
print("[BENCH] hardware_benchmark_flow.py: file loaded, importing numpy/torch "
      "(torch import can take ~5-15s the first time) ...", flush=True)
import numpy as np
import torch
import torch.nn as nn
print(f"[BENCH] imports OK — numpy {np.__version__}, torch {torch.__version__}", flush=True)
...
# the two machines do byte-identical work.
...
def _single_run(run_idx, sizes, seed):
    """Execute ONE full benchmark iteration and return its KPI dict.

    Phases:
      (a) Training  — heavy load, measured as training throughput (samples/sec). Also serves
          to warm caches/threads before we measure the inference KPIs.
      (b) Inference throughput — large batches under sustained load -> KPI 1 (samples/sec).
          CPU power is sampled across THIS phase -> KPI 3 (watts).
      (c) Inference latency — many single-sample calls -> KPI 2 (per-request ms, p50/p95/mean).
    """
    ...
    # ---- (a.5) OPENVINO CONVERSION ------------------------------------------------------
    _dbg(f"run[{run_idx}]: converting PyTorch model to OpenVINO IR...")
    
    # Initialize the OpenVINO Core runtime
    core = ov.Core()
    
    # Convert the PyTorch model directly in memory
    # We provide a dummy tensor to help OpenVINO trace the input shape
    dummy_input = torch.randn(1, 3, 32, 32)
    ov_model = ov.convert_model(model, example_input=dummy_input)
    
    # ---- Setup Throughput Model (Batch Size = ibs) ----
    ibs = sizes["infer_batch"]
    ov_model.reshape([ibs, 3, 32, 32])
    compiled_model_tp = core.compile_model(ov_model, "CPU")
    infer_request_tp = compiled_model_tp.create_infer_request()
    
    # ---- Setup Latency Model (Batch Size = 1) ----
    ov_model.reshape([1, 3, 32, 32])
    compiled_model_lat = core.compile_model(ov_model, "CPU")
    infer_request_lat = compiled_model_lat.create_infer_request()


# ---- (b) INFERENCE THROUGHPUT  (KPI 1) + POWER (KPI 3) ------------------------------
    ...
    for b in range(n_ibatches):
        # Extract the batch and run OpenVINO inference
        batch_data = x_inf_np[b * ibs:(b + 1) * ibs]
        _ = infer_request_tp.infer({0: batch_data})
...
# ---- (c) INFERENCE LATENCY  (KPI 2) -------------------------------------------------
    _dbg(f"run[{run_idx}]: LATENCY phase start — {sizes['latency_warmup']} warmup + "
         f"{sizes['latency_iters']} timed single-sample calls ...")
    
    # Take a single sample and convert to NumPy
    single_np = x_inf_np[:1]
    
    for _ in range(sizes["latency_warmup"]):                 # warm up, don't time these
        _ = infer_request_lat.infer({0: single_np})
...
def func(
    ...
    _dbg(f"averaging {num_runs} run(s): throughput={avg_throughput:.1f} "
        ...
        #  f" power={avg_power if avg_power is None else round(avg_power, 2)}"
         )
